      ...
      if($result->num_rows > 0){
       ...
       <tbody>';
        while($row = $result->fetch_assoc()){
          echo '<tr>';
          echo '<th scope="row" class="text-white">'.$row["stu_id"].'</th>';
          echo '<td class="text-white">'. $row["stu_name"].'</td>';
          echo '<td class="text-white">'.$row["stu_email"].'</td>';
          echo '<td class="text-white"><form action="editstudent.php" method="POST" class="d-inline"> <input type="hidden" name="id" value='. $row["stu_id"] .'><button type="submit" class="btn btn-info mr-3" name="view" value="View"><i class="fas fa-pen"></i></button></form>  
